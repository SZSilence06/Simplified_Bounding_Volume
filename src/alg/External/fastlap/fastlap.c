...
int fastlap(plhsSize,prhsSize,pnumSing,px,pshape,pdtype,plhsType,prhsType,plhsIndex,prhsIndex,plhsVect,prhsVect,pxf,pxnrm,pnumLev,pnumMom,pmaxItr,ptol,pjob)
...
/*  matSetup(sys,size,snglist);*/
  ...
  fprintf(stdout, "  Total memory allocated: %d kilobytes ", memcount/1000);
  uallocEfcy(memcount);
 ...
 */
snglrty *loadSnglrty(numSing,px,pshape,plhsType,prhsType)
int numSing;
double *px;
int *pshape, *plhsType, *prhsType;
 ...
 * 3-space coordinates.
 */
fieldpt *loadFieldpt(size,px,dtype,xnrm)
int size, *dtype;
double *px, *xnrm;
...
int ONsolve(sys, snglist, fptlist, size, maxiter, tol)
ssystem *sys;
snglrty *snglist;
fieldpt *fptlist;
int maxiter, size;
double *tol;
{
  ...
  /* P is the "psuedo-snglrty" for multipole. Ap is the "psuedo-potential". */
  ASSERT(sys->q != NULL);
  ASSERT(sys->p != NULL);
  p = sys->q;
  ap = sys->p;
 ...
 */
int gmres(sys, snglist, fptlist, p, r, ap, z, bv, bh, size, maxiter, tol)
ssystem *sys;
snglrty *snglist;
fieldpt *fptlist;
double *p, *ap, *r, *z, *tol;
...
int size, maxiter;
{
  ...
  double *c=NULL, *s=NULL, *g=NULL, *y=NULL, *work=NULL;
  starttimer;

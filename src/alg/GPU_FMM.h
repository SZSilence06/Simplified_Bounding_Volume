...
namespace SBV
{
    ...
    private:
        ...
        __device__ double GPU_legendre_p(int n, int m, double x);
        __device__ inline static void GPU_toSphericalCoordinate(const Eigen::Vector3d& x, double& r, double& theta, double& phi);
        __device__ inline thrust::complex<double> GPU_R(const Eigen::Vector3d& x, int m, int n);
        __device__ inline thrust::complex<double> GPU_S(const Eigen::Vector3d& x, int m, int n);
        __device__ void GPU_M2L(const GPU_MultipoleExp &inputMoment, const Eigen::Vector3d &xc, const Eigen::Vector3d &x0, GPU_LocalExp &result);
        __device__ void GPU_L2L(const GPU_LocalExp &inputMoment, const Eigen::Vector3d &x0, const Eigen::Vector3d &x1, GPU_LocalExp &result);
